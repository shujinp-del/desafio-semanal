...
function atualizarMetricas() {
  ...
  if (homeMensal)
    homeMensal.innerText =
      `R$ ${total}`;

  if (totalSemana)
    totalSemana.innerText =
      `R$ ${total}`;

  if (totalCorridas)
    totalCorridas.innerText =
      corridas;
}
...
async function resetSemana() {
  ...
  if (
    !confirm(
      "Deseja apagar TODAS as corridas do Firebase?"
    )
  ) {
    return;
  }
